...
def animate_path(
    ...
    for step, pos in enumerate(path):
        ...
        print_map(warehouse_map, robot_pos=pos, path=path[:step + 1], title=title)
        time.sleep(delay)
...
def animate_pygame(
    ...
    cell_size: int = 30,
    fps: int = 10,
    title: str = "StyleWarehouse Robot Navigation",
) -> None:
    """
    ...
    try:
        ...
        print("Falling back to terminal animation.")
        animate_path(warehouse_map, path)
        return
    ...
    for step, robot_pos in enumerate(path):
        ...
        # Step info
        label = font.render(f"Step {step + 1}/{len(path)}  pos={robot_pos}", True, (0, 0, 0))
